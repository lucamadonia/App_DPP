// Dump all Fambliss products in our DB with their current image state so we
// can match them to Shopify products by title/handle and backfill the image.
...
console.log(JSON.stringify(await q(`
  SELECT
    ...
    p.gtin,
    p.image_url,
    (SELECT url FROM product_images WHERE product_id = p.id AND is_primary = true LIMIT 1) AS gallery_primary
  FROM products p
