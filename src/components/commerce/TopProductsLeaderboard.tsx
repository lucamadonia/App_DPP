  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-xl">
      ...
      <ol className="divide-y divide-white/5">
        ...
        {entries.map((e, i) => (
          <LeaderboardRow key={e.productId || e.title} entry={e} index={i} t={t} />
        ))}
      </ol>
    </div>
  );
...
interface LeaderboardRowProps {
  entry: CommerceLeaderboardEntry;
  index: number;
  t: (key: string) => string;
}

function LeaderboardRow({ entry: e, index: i, t }: LeaderboardRowProps) {
  const rowContent = (
    <>
      <span className="hidden xs:inline-block font-display tabular-nums text-2xl text-white/30 w-7 text-right">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex items-center gap-2 text-[11px] text-white/45 flex-wrap">
              <span>
                {new Intl.NumberFormat('de-DE').format(e.unitsSold)} {t('units')}
              </span>
  ...
  const rowClass = 'grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 sm:px-5 py-3.5';
  ...
  return (
    <motion.li
      ...
      className={cn(
        'transition-colors',
        e.productId && 'hover:bg-white/[0.03] active:bg-white/[0.04]',
      )}
    >
      {e.productId ? (
        <Link to={`/products/${e.productId}`} className={rowClass}>
          {rowContent}
        </Link>
      ) : (
