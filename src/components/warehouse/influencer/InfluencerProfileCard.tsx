...
export function InfluencerProfileCard({ contact, isVisible, className }: InfluencerProfileCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="h-12 sm:h-16 bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-blue-500/20" />
      ...
      <CardContent className="p-3 sm:p-4 pt-0">
        {/* Avatar */}
        <div className="-mt-7 sm:-mt-8 mx-auto h-14 w-14 sm:h-16 sm:w-16 rounded-full bg-gradient-to-br from-pink-500 to-purple-500 ring-4 ring-background flex items-center justify-center text-white text-base sm:text-lg font-semibold">
          {getInitials(contact.contactName)}
        </div>

        {/* Name */}
        <p className="font-semibold text-center mt-1.5 sm:mt-2 text-sm sm:text-base truncate">{contact.contactName}</p>
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:gap-2 mt-2 sm:mt-3">
          <div className="text-center p-1.5 sm:p-2 rounded-md bg-muted/50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 bg-gradient-to-t from-card via-card to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-1.5 sm:gap-2">
          <Button
            variant="outline"
            size="sm"
